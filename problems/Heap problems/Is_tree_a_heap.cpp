...
int countnodes(struct Node* root)//count total how many nodes are there
{
    if(root==NULL)
    {
        return 0;
...
bool isCBT(struct Node* root,int index,int totalcount){
    ...
    bool left=isCBT(root->left,index*2 +1,totalcount);
    bool right=isCBT(root->right,index*2+2,totalcount);
    return (left && right);
}
